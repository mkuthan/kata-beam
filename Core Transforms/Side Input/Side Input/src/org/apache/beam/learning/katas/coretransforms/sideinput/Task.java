...
public class Task {
  ...
  static PCollection<Person> applyTransform(
    ...
    return persons.apply(ParDo.of(new DoFn<Person, Person>() {
      @ProcessElement
      public void processElement(@Element Person person, OutputReceiver<Person> out, ProcessContext context) {
        Map<String, String> citiesToCountriesMap = context.sideInput(citiesToCountriesView);

        String country = citiesToCountriesMap.get(person.getCity());
        out.output(new Person(person.getName(), person.getCity(), country));
      }
