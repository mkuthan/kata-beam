...
public class Task {
  ...
  static PCollection<String> applyTransform(PCollection<String> input) {
    return input.apply(ParDo.of(new SplitFn()));
  }

  static class SplitFn extends DoFn<String, String> {

    @ProcessElement
    public void processElement(@Element String line, OutputReceiver<String> words) {
      Arrays.stream(line.split(" ")).forEach(word -> words.output(word));
    }
